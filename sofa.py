...
class MotionController(object):
	...
	def updateJoystick(self, joystick):
...
class NormalMC(MotionController):
	...
	def decodeSubmode(self, turn_angle):
		submode = 'NONE'
		start_angle = 0
		end_angle = 0
		start_m1_speed = 0
	...
	def processUpdate(self):
		turn_direction, turn_angle = self.decodeTurn()

		self._submode, m1_speed, m2_speed, accel_profile = self.decodeSubmode(turn_angle)

		max_speed = self.maxSpeed(turn_angle)

		if self._submode != 'COAST':
			speed = gamma(magnitude) / 100.0
		else:
			speed = 0.0
...
class ComplexMotionController(object):
	...
	def updateJoystick(self, joystick):
		...
		if self._controller and not self._controller.active():
			self._controller = None
...
class Sofa(object):
	...
	def dump_status(self, joystick, controller):
		...
		print("{:10s} {:10s} {:3d} {:3d} {:4f} {:4f}".format(controller.name(), controller.submode(), joystick.magnitude(), joystick.angle(), left_motor, right_motor))
	...
	def control_loop(self):
		...
		while True:
			...
			controller.updateJoystick(joystick)
